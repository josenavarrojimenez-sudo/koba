...
2.  **Send via Gateway:**
    ...
    *   **If using Hermes Baileys (CURRENT):**
        Use `hermes message send --channel whatsapp --to +{USER_PHONE} --media /tmp/koba_response.ogg`
        Or the bridge remote: `{"k": "limon8080", "c": "hermes whatsapp send --to +{PHONE} --media /tmp/koba_response.ogg"}`
